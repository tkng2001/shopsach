...
----TẠO BẢNG TẠM TOÀN CỤC CHO BẢNG CHITIETDONDATHANG
CREATE TABLE temp_CHITIETDONDATHANG
(
	MADONDATHANG int NOT NULL,
	MASACH CHAR(10) NOT NULL,
	SOLUONG INT,
...
--TẠO BẢNG TẠM TOÀN CỤC CHO BẢNG CHITIETHD
CREATE TABLE temp_CHITIETHD
(
	MAHD int NOT NULL,
	MASACH CHAR(10) NOT NULL,
	SOLUONG INT,
...
---Trigger xoá sách các bảng liên quan cũng xoá theo
...
BEGIN
	...
	DELETE CHITIETDONDATHANG WHERE MASACH=@MASACH
	DELETE CHITIETHD WHERE MASACH=@MASACH
...
BEGIN
	...
	select @count=count(*) from sach where matheloai=@matheloai
	while(@count>0)
	begin
...
DELETE temp_CHITIETHD WHERE MAHD=@MAHD
...
DELETE temp_CHITIETDONDATHANG WHERE MADONDATHANG=@MADDH
...
------trigger insert dữ liệu vào 4 view khi dữ liệu thật được insert vào 4 bảng
go
CREATE trigger insertVWCTDonDatHang on chitietdondathang
for insert
as
begin
	declare @MADONDATHANG int,@MASACH char(10),@SOLUONG int,@DONGIA float,@GIAMGIA float,@THANHTIEN float
	select @MADONDATHANG=MADONDATHANG,@MASACH=masach,@SOLUONG=SOLUONG,@DONGIA=DONGIA,@GIAMGIA=GIAMGIA,@THANHTIEN=THANHTIEN from inserted
	insert into temp_CHITIETDONDATHANG values(@MADONDATHANG,@MASACH,@SOLUONG,@DONGIA,@GIAMGIA,@THANHTIEN)
end
go
------TẠO BẢNG tạm CHITIETHD
CREATE trigger insertVWCTHD on chitiethd
for insert
as
begin
	declare @MAHD int,@MASACH char(10),@SOLUONG int,@DONGIA float,@GIAMGIA float,@THANHTIEN float
	select @MAHD=MAHD,@MASACH=masach,@SOLUONG=SOLUONG,@DONGIA=DONGIA,@GIAMGIA=GIAMGIA,@THANHTIEN=THANHTIEN from inserted
	insert into temp_CHITIETHD values(@MAHD,@MASACH,@SOLUONG,@DONGIA,@GIAMGIA,@THANHTIEN)
end
go

--insert into dondathang(ngaytao,thanhtien,diachinhanhang,email,tinhtrang)  values('1-2-2',4,null,'duy123@gmail.com','0')
